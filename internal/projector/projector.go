...
// GeneratedMarker is the first line of every file this package writes.
//
// It is a CONSUMER CONTRACT, not decoration. The dashboard's execution-state
// reader refuses to consume a document at the projection path that does not
// begin with exactly this line, because that path can also hold a
// hand-maintained governance record and guessing which of two producers owns a
// file is the collision the split path exists to prevent.
//
// The pilot that found this watched a project report "0 of 7 deliverables
// complete" over a published projection saying six of them were done. The
// document was correct, readable and current; the reader was correctly
// declining to trust a file that never identified itself. Nothing anywhere
// said so, because a reader that ignores a document it does not recognize has
// nothing to report.
...
func Render(s *State) ([]byte, error) {
	...
	fmt.Fprintf(&b, "%s", GeneratedMarker)
