...
// path inside a wantCommandArgv expectation. The real path is a per-test temp
// dir, so the token matches structurally and the exact suffix is checked by the
// wantSettingsArg assertion.
...
type phase2ProviderCase struct {
	...
	// Preferred over wantCommandPrefix: an exact vector proves which flags are
	// present, their values, and the absence of duplicate or conflicting ones,
	// without pinning cosmetic adjacency.
	...
	all := []phase2ProviderCase{
		{
			...
			family:    "claude",
			// Full argument vector rather than a prefix: the bounded-auto
			// permission mode carries its lifecycle allowlist between
			// --permission-mode and --effort, and adjacency of those two flags
			// is not a CLI contract (verified against claude 2.1.226 --
			// --permission-mode/--allowedTools/--effort/--settings parse
			// independently of each other's order). What must hold is the exact
			// set of flags and values, which this pins.
			//
			// The allowlist is spelled out literally rather than referencing
			// workerbuiltin.ClaudeBoundedAutoAllowedToolsArg: an expectation
			// tracking the production constant would follow it if the grant were
			// ever widened (e.g. to bare Bash or Bash(gc:*)) and still pass.
			wantCommandArgv: []string{
				"claude",
